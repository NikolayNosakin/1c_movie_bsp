...
Процедура ПоказатьДлительнуюОперацию(Показать)	
	Элементы.Группа1.Видимость = НЕ Показать;
	Элементы.ВосстановитьДанные.Видимость = НЕ Показать;
	Элементы.СохранитьДанные.Видимость = НЕ Показать; 
	Элементы.СкрыватьПросмотренныеСерии.Видимость = НЕ Показать;
...
Процедура Восстановить(ПутьКФайлу)	
	...
	Если Лог = "" Тогда
		...
		ПоказатьВопрос(Оповещение, "Восстановление данных завершено. Для нормальной работы желательно перезапустить программу. Перезапустить?", РежимДиалогаВопрос.ДаНет,10, КодВозвратаДиалога.Да,,КодВозвратаДиалога.Да);	
	Иначе
...
Процедура КодАвторизации(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ЯДФормаАвторизации", ПараметрыОткрытия,Элементы.ЯД_КодАвторизации, УникальныйИдентификатор);
КонецПроцедуры
